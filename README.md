...
- **总开关**：控制面板一键开启/暂停续火花；拨到关闭可选择**暂停 1 天、自定义天数或永久**，到期自动恢复，状态实时可见
- **好友冻结**：双击好友行即可冻结某位好友（列表保留但暂停发送，行末显示 stop 标记），再双击一次恢复
...
- **断网自动补发**：发送前先等网络就绪（覆盖唤醒后 Wi-Fi 慢重连）；若仍失败，Windows 检测到「网络已连接」会自动重新拉起程序补发——无需软件常驻后台、无重试次数上限
...
- **企业微信群机器人远程提醒**（可选）：粘贴 Webhook 地址即可；**每日发送成功、当天最终失败、晚间补发、晚间仍未成功**四类事件均按开关推送固定模板消息（统一署名「—— 来自 SparkAK」）；推送可 **@ 指定的群成员**（在「推送设置」里填你在群里的名字，已取消 @所有人，避免打扰其他人）。钉钉群机器人文本格式兼容
...
1. 完全关闭正在运行的程序
2. 将新版压缩包内的 `SparkAK` 文件夹**直接覆盖解压到原目录**（选择「替换目标中的文件」）
3. 完成——好友配置、登录状态、发送记录全部自动保留

> ⚠️ **v1.4.3 及更早版本的用户请注意**：由于 v1.4.4 起安装包内目录由 `app/`
...
| `notify.remind_time` | 每晚提醒检查时间（需晚于发送时间最大可能值） | `22:30` |
| `notify.auto_resend` | 晚间检查时先自动补发一次 | `true` |
| `notify.notify_on_fail` | 当天发送最终失败时立即推送 | `true` |
| `notify.notify_on_success` | 每日发送全部成功时也推送 | `true` |
| `notify.mention_names` | 推送时 @ 的群成员名（多人用顿号分隔；留空不 @） | 空 |
